...
console.log('===================>> Soal 1')
    ...
    newData.forEach(function(d)  {
        console.log('ID         : ', d.idTransaction);
        console.log('Location   : ', d.storeLocation);
        console.log('Email      : ', d.userEmail);
        console.log('Purchase   : ', d.purchaseMethod);
...
let groupedData = groupBy(data, 'storeLocation')
console.log(groupedData)

  // soal no 3
  console.log('===================>> Soal 3');
  const profitTransaksi = data.map(function(d) {
    let profit = 0;
      d.items.forEach(function(i) {
        profit += i.price.$numberDecimal * i.quantity.$numberInt;
        });
        return {
        idTransaction: d.id,
        profit: profit,
...
console.log(profitTransaksi)
  
 
  // soal no 4
  console.log('===================>> Soal 4');
    const jumlah = data.map((d) => {
      let jumlahBarang = 0;
      d.items.forEach((i) => {
        jumlahBarang += parseInt(i.quantity.$numberInt);
      });
      return {
        idTransaction: d.id,
        jumlahBarang,
...
const rating  = []
    data.forEach((d) => {
        rateSatisfactions = d.customer.satisfaction.$numberInt;
        const toko = d.storeLocation;
        let ratings = 0;
          const avgTotal =
            
            parseInt(rateSatisfactions.length)/
            parseInt(rateSatisfactions);
          ratings += avgTotal;
...
const profitToko = [];
    data.forEach((d) => {
        ...
        d.items.forEach((i) => {
          const profitTotal =
            parseFloat(i.price.$numberDecimal) *
            parseInt(i.quantity.$numberInt);
          profit += profitTotal;
        });
        if (profitToko[toko]) {
            profitToko[toko] += profit;
          } else {
            profitToko[toko] = profit;
          }
      });

console.log(profitToko);